...
只收录已由本地样例直接支撑的模板。这里的骨架不是完整成品，而是可填空的最小 DSL 框架。
...
- 本页骨架的节点组合来自本地 fixture，但 app DSL 的版本号已按当前导出标准归一化到 `0.6.0`。
...
- 只把本页骨架用于 `template-validation-status` 中标记为 `已由源码样例直接验证` 的模板。
