...
def validate_expectation(expectation: dict, text: str) -> list[str]:
    errors: list[str] = []

    must_contain_all = expectation.get("must_contain_all", [])
    for marker in must_contain_all:
        if marker not in text:
            errors.append(f"缺少必须包含内容: {marker}")
...
def main() -> int:
    ...
    errors = validate_expectation(expectation, text)
