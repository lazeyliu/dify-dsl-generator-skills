...
FOUNDATIONS_ROUTE = "skills/dify-dsl-foundations/SKILL.md"
NODES_ROUTE = "skills/dify-dsl-nodes/SKILL.md"
TEMPLATES_ROUTE = "skills/dify-dsl-templates/SKILL.md"
QUALITY_ROUTE = "skills/dify-dsl-quality/SKILL.md"
GOVERNANCE_ROUTE = "skills/dify-dsl-governance/SKILL.md"
...
def matches_template_skeleton(
    ...
    if kind == "app" and mode == "advanced-chat":
        if node_count == 3 and edge_count == 2:
            return node_types == Counter({"start": 1, "llm": 1, "answer": 1})

        if node_count == 4 and edge_count == 3:
            return node_types == Counter(
                {
                    "start": 1,
    ...
    if kind == "app" and mode == "workflow":
        if node_count == 3 and edge_count == 2 and node_types.get("start") == 1 and node_types.get("end") == 1:
            middle_types = {"llm", "code", "tool", "http-request", "template-transform"}
            return sum(node_types.get(node_type, 0) for node_type in middle_types) == 1

        if node_count == 4 and edge_count == 3:
            if node_types.get("start") == 1 and node_types.get("http-request") == 1 and node_types.get("end") == 1:
                return node_types.get("tool") == 1 or node_types.get("code") == 1

    if kind == "rag_pipeline" and node_count == 3 and edge_count == 2:
        if node_types.get("datasource") == 1 and node_types.get("knowledge-index") == 1:
            return node_types.get("document-extractor") == 1 or node_types.get("tool") == 1
...
def classify_routes(
    ...
    conversation_var_count: int,
    node_count: int,
    edge_count: int,
) -> list[str]:
    routes = [FOUNDATIONS_ROUTE]

    if node_types:
        routes.append(NODES_ROUTE)

    if matches_template_skeleton(kind, mode, node_types, node_count, edge_count):
        routes.append(TEMPLATES_ROUTE)
    ...
    if kind == "app" and mode == "advanced-chat":
        if "question-classifier" in node_types or "knowledge-retrieval" in node_types or conversation_var_count > 0:
            ...
            routes.append(GOVERNANCE_ROUTE)

    if dependency_count > 0 or conversation_var_count > 0:
        if GOVERNANCE_ROUTE not in routes:
            routes.append(GOVERNANCE_ROUTE)
...
def render_report(path: Path, data: dict) -> str:
    ...
    if TEMPLATES_ROUTE not in analysis["routes"]:
        lines.append("- 这个样本不会直接覆盖模板 skill；模板路线仍需单独样本补齐。")
