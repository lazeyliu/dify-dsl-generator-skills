...
description: 生成、优化、调优、调整、校验与修复 Dify Workflow、Chatflow、RAG Pipeline DSL 编排的技能。用于用户要求创建新 DSL、修改现有 DSL、按节点重排工作流、选择模板或模板变体、补齐必填字段和选择器、降低成本、提高稳定性、修复错误链路、输出已验证骨架、生成审核报告时。
---
...
- 若未显式指定，默认策略按 `strict` 解释；覆盖率通过标准为适用路径 `>= 95%`，且主路径、fallback 路径、外部异常路径不得缺测；默认输出形态为 `JSON`，可选 `纯文本`、`Markdown`、`HTML`。
